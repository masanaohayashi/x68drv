...
| Stage | 内容 | DiFinder 参照箇所 |
|-------|------|-------------------|
| A | CLI inject 1 ファイル（実験フラグ） | `SaveFile` / `AllocateUnitGroups` |
| B | delete / overwrite + fsck | `DeleteFile` |
| C | mkdir / rename | dir Expand / rename UI 経路 |
| D | FUSE experimental-write | UI import をコールバックに載せ替え |
